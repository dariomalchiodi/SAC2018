...
def estimate_possibility(n, mu, k, g, cs, ks, num_holdouts, percentages,
                         fuzzifier, verbose=False, crisp=False):
    ...
    for h in range(num_holdouts):
        ...
        with open('data/crisp/axioms-results-holdout-{}-{}-details.csv'.format(fuzzifier.name, h), 'w') as output_file:
            writer = csv.writer(output_file)
            writer.writerows(results)

        with open('data/crisp/axioms-results-holdout-{}-{}-global.csv'.format(fuzzifier.name, h), 'w') as output_file:
            writer = csv.writer(output_file)
            writer.writerows([
        ...
        plt.savefig('data/crisp/axioms-results-holdout-{}-{}-boxplot.png'.format(fuzzifier.name, h))
        plt.clf()

        p = plt.hist(errors, bins=50)
        plt.savefig('data/crisp/axioms-results-holdout-{}-{}-histogram.png'.format(fuzzifier.name, h))
        plt.clf()
    ...
    with open('data/crisp/axioms-results-holdout-{}-average-metrics.csv'.format(fuzzifier.name), 'w') as output_file:
        writer = csv.writer(output_file)
        writer.writerows([
...
for f in fuzzifiers:
    ...
    estimate_possibility(n, mu, k, g, cs, ks, num_holdouts, percentages, f, verbose=True, crisp=True)
